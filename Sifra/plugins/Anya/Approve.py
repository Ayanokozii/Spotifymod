...
@Anya.on_callback_query(filters.regex("close"))
...
@Anya.on_callback_query(filters.regex("gib_source"))
...
CHAT_ID = [int(Anya) for Anya in chat_id_env.split(",")] if chat_id_env else []
...
@Anya.on_chat_join_request((filters.group | filters.channel) & filters.chat(CHAT_ID) if CHAT_ID else (filters.group | filters.channel))
async def autoapprove(client: Anya, message: ChatJoinRequest):
    chat = message.chat  # Chat
    user = message.from_user  # User
    photo = await Anya.download_media(user.photo.big_file_id)
